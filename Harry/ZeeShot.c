...
char *Init()
{
  ...
  if(arrayLen(ZPT_Storage_Get(admin))!=0) return false;
...
char *invoke(char *method, char *args)
{
  ...
  if (strcmp(method ,"changeAdmin")==0 ){
    if(ZPT_Runtime_CheckWitness(ZPT_Storage_Get(admin))==0) return false;
    struct Params{
      char * address;
    };
    struct Params *p = (struct Params *)malloc(sizeof(struct Params));
  ...
  if (strcmp(method ,"setAdmin")==0 ){
    if(ZPT_Runtime_CheckWitness(CEOAddress)==0) return false;
    struct Params{
      char * address;
    };
    struct Params *p = (struct Params *)malloc(sizeof(struct Params));
