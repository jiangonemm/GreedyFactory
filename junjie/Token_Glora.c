...
char * increaseTotal(char * valueChar){
    ...
    long long ceoBalance = Atoi64(ZPT_Storage_Get(ceoAddress)) + value;
    ZPT_Storage_Put("totalSupply", I64toa(totalSupplyNew,10));
    ZPT_Storage_Put(ceoAddress, I64toa(ceoBalance,10));
...
char * decreaseTotal(char * valueChar){
    ...
    if (totalSupply <= value)
        return "20018";
    long long ceoBalance = Atoi64(ZPT_Storage_Get(ceoAddress));
    if (ceoBalance < value)
        return "20019";
